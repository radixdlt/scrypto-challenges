...
fn test_create_proposal() {
    ...
    let create_proposal_manifest = ManifestBuilder::new(&NetworkDefinition::simulator())
        .call_method(account_component, "lock_fee", args!(dec!("100")))
        .call_method(
            component,
            "create_proposal",
    ...
    let receipt = test_runner.execute_manifest(create_proposal_manifest, vec![public_key.into()]);
...
#[test]
fn test_error_create_proposal_not_minimum_voting_time() {
    let mut store = TypedInMemorySubstateStore::with_bootstrap();
    let mut test_runner = TestRunner::new(true, &mut store);
...
#[test]
fn test_error_deposit_proposal_voting_period_ended() {
    let mut store = TypedInMemorySubstateStore::with_bootstrap();
    let mut test_runner = TestRunner::new(true, &mut store);
    ...
    test_runner.set_current_epoch(70);
    let receipt = test_runner.execute_manifest(deposit_proposal_manifest, vec![public_key.into()]);
