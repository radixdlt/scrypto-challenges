...
fn test_resolve_proposal() {
    ...
    let membership_id_2: u64 = 12u64;
    let membership_id_3: u64 = 34u64;
    let membership_id_4: u64 = 19u64;
    ...
    let vote_proposal_manifest = ManifestBuilder::new(&NetworkDefinition::simulator())
        ...
        .call_method(
            ...
            args!(
                NonFungibleId::from_u64(proposal_id),
                NonFungibleId::from_u64(membership_id_2),
                ("DAN-DAN".to_string(), dec!("35"))
            ),
        )
        .call_method(
            component,
            ...
            args!(
                NonFungibleId::from_u64(proposal_id),
                NonFungibleId::from_u64(membership_id_3),
                ("DAN-COIN".to_string(), dec!("43"))
            ),
        )
        .call_method(
            component,
            ...
            args!(
                NonFungibleId::from_u64(proposal_id),
                NonFungibleId::from_u64(membership_id_4),
                ("COIN-DAN".to_string(), dec!("43"))
            ),
        )
        .call_method(
            account_component,
    ...
    test_runner.execute_manifest(vote_proposal_manifest, vec![public_key.into()]);

    let resolve_proposal_manifest = ManifestBuilder::new(&NetworkDefinition::simulator())
        .call_method(account_component, "lock_fee", args!(dec!("100")))
        .call_method(
            component,
            "resolve_proposal",
            args!(NonFungibleId::from_u64(proposal_id)),
        )
        .build();

    test_runner.set_current_epoch(100);
    let receipt = test_runner.execute_manifest(resolve_proposal_manifest, vec![public_key.into()]);
...
#[test]
fn test_error_resolve_proposal() {
    let mut store = TypedInMemorySubstateStore::with_bootstrap();
    let mut test_runner = TestRunner::new(true, &mut store);
    ...
    test_runner.set_current_epoch(20);
    test_runner.execute_manifest(vote_proposal_manifest, vec![public_key.into()]);
    ...
    test_runner.set_current_epoch(49);
    let receipt = test_runner.execute_manifest(resolve_proposal_manifest, vec![public_key.into()]);
