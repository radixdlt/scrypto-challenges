...
fn test_claim_rewards() {
    ...
    let channel_id: String = String::from("f43f4f3cr");
    ...
    test_runner.execute_manifest(subscribe_manifest, vec![public_key_2.into()]);

    let claim_rewards_manifest = ManifestBuilder::new(&NetworkDefinition::simulator())
        .call_method(account_component_2, "lock_fee", args!(dec!("10")))
        .withdraw_from_account_by_ids(
            &membership_2_bt_id,
            mint_membership_address,
        ...
        .take_from_worktop_by_ids(
            ...
            |build, membership_bucket_id| {
                build.call_method(
                    component,
                    "claim_rewards",
                    args!(channel_id, Bucket(membership_bucket_id)),
                )
            },
        )
        .call_method(
    ...
    test_runner.set_current_epoch(26);

    let recept = test_runner.execute_manifest(claim_rewards_manifest, vec![public_key_2.into()]);
...
#[test]
fn test_error_claim_rewards_membership_not_suscribe() {
    let mut store: TypedInMemorySubstateStore = TypedInMemorySubstateStore::with_bootstrap();
    ...
    let channel_id: String = String::from("1234567");
...
#[test]
fn test_error_claim_rewards_epoch_error() {
    let mut store: TypedInMemorySubstateStore = TypedInMemorySubstateStore::with_bootstrap();
    ...
    test_runner.set_current_epoch(21);
