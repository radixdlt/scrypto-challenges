...
use crate::voting_system::VoteCount::{FungibleVoteCount, NonFungibleVoteCount};
...
impl VoteConfig {
    ...
    pub fn new_proposal(
        ...
        options.insert(
            ...
            VoteOption {
                description: None,
                code_executions,
                requirement: approve_requirement.clone(),
            },
        );
        ...
        options.insert(
            ...
            VoteOption {
                description: None,
                code_executions: vec![],
                requirement: reject_requirement,
            },
        );
    ...
    fn assert_options_valid(options: &HashMap<String, VoteOption>) {
        ...
        assert!(
            fallback_count <= 1,
            "Only one option with WinRequirement::Fallback is allowed"
        );
    ...
    fn get_win_requirement_discriminant(
        ...
        if discriminates.len() > 1 {
            ...
            discriminates
                .into_iter()
                .next()
                .ok_or("No options with requirements other than fallback found")
        }
    }
}
        ...
        match &mut self.state {
            VoteState::Open(ref mut vote_count) => match vote_count {
                FungibleVoteCount(ref mut voting_powers) => {
                    voting_powers
                        .entry(option_name)
                        .and_modify(|voting_power| *voting_power -= amount);
                }
...
impl WinRequirement {
    /// Creates a new AbsolutRatio or RelativeRatio requirement where a majority of the votes (>50%)
    /// must be accumulated by an option in order for it to win
    ///
    /// Arguments:
    /// - `absolut`: Whether to create an absolut (true) or a relative (false) requirment
    ///
    /// Returns: the created requirement
    pub fn majority(absolut: bool) -> Self {
        let value = dec!("0.5") + one_atto();
        if absolut {
            Self::AbsolutRatio(value)
        } else {
            Self::RelativeRatio(value)
        }
    }
...
pub struct EvaluationSettings {
    ...
    allow_multiple_winning_options: bool,
}
...
impl VoteState {
    ...
    fn to_decided(&self, winning_options: Vec<String>) -> Self {
        match self {
            VoteState::Open(vote_count) => VoteState::Decided {
                winning_option_names: winning_options,
...
impl VoteCount {
    ...
    pub fn get_voting_power_per_option(&self) -> HashMap<String, Decimal> {
        match self {
            ...
            NonFungibleVoteCount(member_ids) => member_ids
                .iter()
                .map(|(option, ids)| (option.to_owned(), Decimal::from(ids.len())))
                .collect(),
        }
    }
}
...
/// Returns a [Decimal] value equal to one Atto, the smallest possible Decimal value.
fn one_atto() -> Decimal {
