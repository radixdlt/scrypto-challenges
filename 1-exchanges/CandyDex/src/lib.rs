    ...
    impl CandyDex {
        pub fn new(fee: Decimal) -> (Component,Bucket) {
            ...
            fn meta_amounts(
                ...
                // consequentely metaCandy amount, burn metaBadge if all liquidity is removed. 
                if flag == dec!(0) {
            ...
            fn candyput_pri(
                ...
                // Adjust candy amount adding protocol fee if needed by identified calling method.
                if prc_in == dec!(0) { 
                    candy_amnt = CandyDex::adjust_fee(self, candy_amnt); 
                }
                
                // Call a method to calculate candy output amount.
            ...
            fn candyput_sec(
                ...
                if prc_in == dec!(0) {
                    let amount_in = CandyDex::candy_sum(self, amnt, addr, candy_addr, 2); 
            ...
            fn price_mod(
                ...
                // Retrieve candy reserve amount.
                let total_candy = self.candy_vaults.get(&address).unwrap().amount();
            
                // Determine relative math formula once identified calling method. 
                match flag {
                    1 => total_candy*price/(total_candy-amount),         
                    2 => (total_candy*price + amount*price)/total_candy, 
                    3 => (total_candy*price - amount*price)/total_candy,
                    _ => total_candy*price/(total_candy+amount)
                }
            }
            ...
            fn candy_sum(
                ...
                match flag {
                    ...
                    2 => {
                        let tot_amnt_out = self.candy_vaults.get(&addr_pri).unwrap().amount();
                        tot_amnt_out*candy_out_amnt/(tot_amnt_out-amnt_pri)
                    },
                    _ => {
                        ...
                        if flag == 3 {      
                            ...
                            price_new = CandyDex::price_mod(self, amount, addr_pri, price_in, 2);
                        } else if flag == 0 {
                            price_new = CandyDex::price_mod(self, amount, addr_pri, price_in, 1);
                        }
        ...
        pub fn stock_candy(&mut self, candy: Bucket, new_price: Decimal) -> (Bucket,Bucket) {
            ...
            // Verify Candy to stock isn't XRD, amount is greater than zero, this Candy hasn't  
            // been stocked yet.
            ...
            // Insert Candy address as well as other metadata in relative hashmaps.
            self.candy_map.insert(candy_addr,(name.clone(),symbol.clone(),new_price));
            self.meta_map.insert(candy_addr,(none,none,candy_addr));
            
            // Mint metaCandy, metaBadge and insert relative hashmaps values. 
        ...
        pub fn restock_candy(&mut self, candy: Bucket) -> (Bucket,Bucket) {
            ...
            // Verify Candy to restock already exists within Protocol.
            match self.candy_map.get(&candy_addr) {
                Some((a,b,c)) => 
            ...
            // Mint relative metaCandy tokens.    
        ...
        pub fn unstock_candy(
            ...
            // Check if there's enough Candy to reimburse user otherwise reimburse the difference
            // in XRD.
            ...
            // Burn relative metaCandy, verify user's remaining Candy liquidity is greater then zero
        ...
        pub fn get_candy_sell_amount_bexsc(
            ...
            let new_price: Decimal = 
                CandyDex::price_mod(self, xrd_amount/price, candy_addr, price, 3);
        ...
        pub fn get_xrd_buy_amount_bxsec(
            ...
            let new_price: Decimal = 
                CandyDex::price_mod(self, candy_amnt, candy_addr, price, 0);
        ...
        pub fn get_candy_buy_amount_bcsex(
            ...
            let new_price: Decimal = 
                CandyDex::price_mod(self, xrd_amnt/price, candy_addr, price, 2);
        ...
        pub fn get_xrd_sell_amount_becsx(
            ...
            let new_price: Decimal = 
                CandyDex::price_mod(self,  candy_amount, candy_addr, price, 1);
        ...
        pub fn get_candy_sell_amount_becsc(
            ...
            let amount = CandyDex::candy_sum(self, amount_in, addr_in, addr_out, 2);
        ...
        pub fn buy_candy_sell_exact_xrd(
            ...
            let amount_in = CandyDex::candy_sum(self, xrd_amnt, addr_in, addr_in, 3);
        ...
        pub fn buy_xrd_sell_exact_candy(&mut self, xrd_min: Decimal, candy_out: Bucket) -> Bucket {
            ...
            let new_price: Decimal = 
                CandyDex::price_mod(self, candy_out.amount(), addr, price_out, 0);
        ...
        pub fn buy_exact_xrd_sell_candy(
            ...
            let price_out: Decimal = CandyDex::candyprice(self, candy_out.resource_address());
            let new_price = 
                CandyDex::price_mod(self, xrd_input/price_out, addr, price_out, 3);
