...
blueprint! {
    ...
    impl Market {
        ...
        fn order_ticket(&mut self, order_number: i64, token_address: Address) -> Bucket {
            ...
            self.ticket_minter_badge.authorize(|badge|{
                self.ticket_nft_def.mint_non_fungible(&NonFungibleKey::from(order_number as u128), ticket, badge)
            })
        }
        ...
        fn create_limit_order<F>(&mut self, token: Address, bucket: Bucket, make_order: F) -> Bucket where F: Fn(i64, Bucket) -> Order {
            ...
            let mut order = make_order(order_number, bucket);

            self.fill_order(&mut order);
        ...
        fn fill_order(&mut self, order: &mut Order) {
            ...
            for matched_order in self.matching_orders(order) {
                ...
                Market::fill_matched_order(order, matched_order, price);
        ...
        fn fill_matched_order(order_a: &mut Order, order_b: &mut Order, price: Decimal) {
            ...
            if order_a.is_buy_order() && order_b.is_sell_order() {
                Market::fill_sell_order(order_b, order_a, price);
            } else {
                Market::fill_sell_order(order_a, order_b, price);
            }
        }

        fn fill_sell_order(sell_order: &mut Order, buy_order: &mut Order, price: Decimal) {
        ...
        pub fn withdraw_order(&mut self, ticket_bucket: Bucket) -> (Bucket, Bucket, Bucket) {
            let tickets = ticket_bucket.get_non_fungibles::<OrderTicket>();
            ...
            if index.is_some() {
                ...
                // Usually we would remove this order from the list of orders here
                // because it will be settled after this. However, that's not allowed
                // since Scrypto v0.3, Vaults must not be destroyed.
                // Refactoring this to use shared, persistent vaults will take a bit of time.
                // I shall do this in separate pull request.
        ...
        fn matching_orders(&mut self, order: &Order) -> Vec<&mut Order> {
            let buy = order.is_buy_order();

            let mut orders = (&mut self.orders)
                .into_iter()
                .filter(|o| o.is_buy_order() != buy) // match buy to sell orders and vice versa
                .filter(|o| (buy && Market::is_matching_sell_order(order, *o)) || Market::is_matching_buy_order(order, *o))
        ...
        fn is_matching_sell_order(buy_order: &Order, sell_order: &Order) -> bool {
        ...
        fn is_matching_buy_order(sell_order: &Order, buy_order: &Order) -> bool {
