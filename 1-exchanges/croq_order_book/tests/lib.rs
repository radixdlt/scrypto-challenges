        ...
        .run(
            TransactionBuilder::new(executor)
                .call_function(
                    ...
                    "CroqOrderBook",
                    "instantiate",
                    vec![format!("{}", token), format!("{}", RADIX_TOKEN)],
                    Some(account),
                )
                .call_method_with_all_resources(account, "deposit_batch")
                .build(vec![key])
...
fn create_actor<'a, L: SubstateStore>(
    executor: &mut TransactionExecutor<'a, L>,
...
impl<'a, L: SubstateStore> TestEnv<'a, L> {
    fn new(
        ...
        let actors: Vec<(EcdsaPublicKey, Address)> =
            (0..5).map(|_| create_actor(&mut executor)).collect();
        let package = executor
            .publish_package(include_code!("croq_order_book"))
            .unwrap();
        let market_hand = create_actor(&mut executor);
        let token = create_token(&mut executor, market_hand.1, 10000000.into(), market_hand.0);
        actors.iter().for_each(|(_, account)| {
            transfer_token(
                &mut executor,
                market_hand.1,
                *account,
        ...
        let instance = create_exchange(&mut executor, package, market_hand.1, token, market_hand.0);
        (
            TestEnv {
                executor,
                token,
        ...
        let receipt = self
            ...
            .run(
                TransactionBuilder::new(&self.executor)
                    .call_method(self.instance, "register", vec![], Some(actor.1))
    ...
    fn monitor(&mut self, actor: (EcdsaPublicKey, Address)) {
        let receipt = self
            ...
            .run(
                TransactionBuilder::new(&self.executor)
                    .call_method(self.instance, "monitor", vec![], Some(actor.1))
                    .build(vec![actor.0])
                    .unwrap(),
            )
            .unwrap();
