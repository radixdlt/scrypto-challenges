   ...
   impl StakingPool {
      ...
      pub fn withdraw(&mut self, amount: Decimal) -> FungibleBucket {
         ...
         if deposit_amount == amount {
            ...
            self.running_product *= PreciseDecimal::from(dec!(1) - (amount / self.deposits.amount()));
         }
      ...
      pub fn distribute(&mut self, gain: FungibleBucket) {
         ...
         if self.running_sum.get_mut(&(self.current_epoch)).is_none() {
            self.running_sum.insert(self.current_epoch, PreciseDecimal::ZERO);
         };
      ...
      pub fn redeem(&mut self, id: NonFungibleLocalId) -> (FungibleBucket, FungibleBucket) {
         let snapshot = self.snapshots.remove(&id).expect("SNAPSHOT_NOT_FOUND_ERROR");
         ...
         let compounded_deposit_amount = if snapshot.epoch == self.current_epoch {
            PreciseDecimal::from(snapshot.deposit_amount) * (self.running_product / snapshot.running_product)
         } else {
            PreciseDecimal::ZERO
         };
         ...
         gain_amount = (gain_amount / snapshot.running_product) * (epoch_running_sum - snapshot.running_sum);
         ...
         if self.contribution_counter - self.redemption_counter == 0 {
            (self.deposits.take_all(), self.distributed_resources.take_all())
         } else {
            (
      ...
      fn _is_pool_empty(&self) -> bool {
         self.contribution_counter == self.redemption_counter || self.deposits.amount() == dec!(0)
      }
   }
}
