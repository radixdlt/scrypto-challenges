...
/// Since there is an unstaking delay when we unstake the Liquid Stake Units (LSU), they are burned and a non-fungible claim token (NFT) is then minted, and returned to the unstaker (in our case, the protocole). The claim token has embedded data, the UnstakeData, which defines the expected amount of XRD of the requested unstake, and the time after which that XRD can be claimed. Once the unstaking period has passed, the claim token can be sent back to the validator component, which will burn it and return the appropriate amount of XRD.
#[derive(ScryptoSbor, NonFungibleData)]
...
mod yield_tokenizer {
   struct YieldTokenizer {
      ...
      /// For more details, check comments in staking_pool.rs.
      lsu_pool: Owned<StakingPool>,

      /// Vault storing pending claim NFT fro unstaked XRD
      /// We are using an Option since we don't have the ResourceAddress of the unstake NFT yet. After the first unstake, the Vault is then instantiated.
      claim_vault: Option<NonFungibleVault>,

      /// Claim limit
      /// Max Number of claim NFTs that can claimed at a time
      /// We use a limit to claim the unstake NFTs because when we get the NFTs in a Vault with the non_fungible_local_ids function, we can only select a limited amount.
      claim_limit: u32,
   }
   ...
   impl YieldTokenizer {
      ...
      /// The LSUs are used as input and generates two resources as output: a fungible resource, the sXRD, and a non-fungible one, the Yield Token (YT). The sXRD is a synthetic derivative designed to have roughly the same market value as the XRD. The YT, on the other hand, represent current position in LSU and can be exchanged at any time for sXRD whose value close to the yield generated by the LSUs that were used for its creation.
      ///
      /// # Arguments
      ///
      ...
      /// the minted sXRD.
      /// The 1:1 convertibility with XRD of sXRD offers an opportunity for an external exchange pool between sXRD/XRD. It will participate in maintaining the value of sXRD at 1 XRD by offering direct arbitrage opportunities.To facilitate arbitrage operations, users could also accept XRD deposited in the unstaking vault to create the corresponding sXRD.
      ///
      /// # Arguments
      ///
      ...
      /// In case we don't have enough XRD in the Vault, we then keep checking if some unstake NFTs are claimable, if we still don't have enough XRD we then give the corresponding amount in LSU. We added a boolean in case the user doesn't want LSU returned.
      ///
      ///
      /// # Arguments
      ///
      ...
      /// token's data.
      /// After the claim, the YT is burned and the underlying LSU amount is unstaked. An unstake NFT will be provided and kept to later be used to claim XRD in the Vault.
      ///
      /// # Arguments
      ///
      ...
      /// We go through 'limit' number of NFTs and check if they are claimable, if they are we then save their Ids to claim them just after.
