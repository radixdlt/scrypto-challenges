...
pub fn asset_pool_base() -> Result<(), RuntimeError> {
   ...
   staking_pool.contribute(id_0.clone(), FungibleBucket(lsu.take(dec!(100), &mut env)?), &mut env)?;
   ...
   staking_pool.contribute(id_1.clone(), FungibleBucket(lsu.take(dec!(100), &mut env)?), &mut env)?;
   ...
   staking_pool.contribute(id_2.clone(), FungibleBucket(lsu.take(dec!(200), &mut env)?), &mut env)?;
   ...
   staking_pool.contribute(id_3.clone(), FungibleBucket(lsu.take(dec!(100), &mut env)?), &mut env)?;
   ...
   staking_pool.contribute(id_4.clone(), FungibleBucket(lsu.take(dec!(100), &mut env)?), &mut env)?;
   ...
   staking_pool.contribute(id_5.clone(), FungibleBucket(lsu.take(dec!(200), &mut env)?), &mut env)?;
   ...
   let distribute_result = staking_pool.distribute(FungibleBucket(sxrd.take(dec!(100), &mut env)?), &mut env);
