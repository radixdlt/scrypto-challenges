...
describe('bip39', () => {
	...
	it('should work with every BIP39 supported language', () => {
		...
		languagesSupportedByBIP39.forEach(language => {
			...
			const someWord = wordList[14] // can't use first. since same for chinese traditional and simple.
			expect(someWord).not.toBe(wordFromLastWordlist)
...
const languages = new Map<LanguageT, Vector[]>([
	[LanguageT.ENGLISH, englishVectors],
	[LanguageT.JAPANESE, japaneseVectors],
])
