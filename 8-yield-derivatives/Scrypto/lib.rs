...
pub struct SaleDetail {
    pub dapp_definition_caddy: ComponentAddress,
...
mod super_iyo {
    ...
    impl Super {
        pub fn new(
            ...
            testing: u32,
        ) -> (NonFungibleBucket, Global<Super>, FungibleBucket) {
            ...
            let badge_owner: NonFungibleBucket =
                ResourceBuilder::new_integer_non_fungible(owner_role_component)
                    .metadata(metadata!(
                    init {
            ...
            let badge_db_updater: FungibleBucket =
                ResourceBuilder::new_fungible(owner_component_or_owner_badge.clone())
                    .metadata(metadata!(
                    init {
                        "name" => "SUPER Yield Updater Badge".to_owned(), locked;
                        "dapp_definitions" => dapp_definition_addy_vec.to_owned(), updatable;
                        "icon_url" => current_colors.ww.to_owned(), updatable;
                    }))
                    .mint_roles(mint_roles!(
                    minter => access_rule_component.clone();
                    minter_updater => access_rule_component.clone();
            ...
            let new_super_event: SaleDetailEvent = SaleDetailEvent {
                dapp_definition_caddy: component_addy,
            ...
            let component: Global<Super> = Self {
                ...
                dapp_definition_caddy: dapp_definition_addy_vec.clone(),
                ...
                super_raddy: super_resource_addy,
                super_y_raddy: super_yield_resource_addy,
                super_t_raddy: super_yield_resource_addy, //PLACEHOLDER
                yield_nft_raddy: nft_resource_addy,
            ...
            .metadata(metadata!(
                init {
                    ...
                    "dapp_definition" => dapp_definition_addy.to_owned(), updatable;
        ...
        pub fn start_sale(&mut self, fee: Bucket) -> Bucket {
            ...
            self.sale_details.super_t_raddy = self.super_t_raddy;
        ...
        pub fn set_trustfund_metadata(&mut self, fee: Bucket) -> Bucket {
            ...
            self.pool.set_metadata(
                ...
                self.dapp_definition_caddy[0].to_owned(),
            );

            let pool_token: Bucket = self.pool.contribute(fee);
            self.super_t_raddy = pool_token.resource_address();

            let pool_token_manager: ResourceManager = ResourceManager::from(self.super_t_raddy);
            ...
            pool_token_manager.set_metadata(
                ...
                self.dapp_definition_caddy.to_owned(),
            );
        ...
        pub fn update_trust_metadata(
            ...
            if self.super_t_raddy == self.component_badge_raddy {
        ...
        pub fn check_if_sale_complete(&mut self) {
            ...
            info!("Token sale completed = {}", self.sale_completed);
        ...
        pub fn split_yield_nft(
            &mut self,
            yield_nft: NonFungibleBucket,
            number_of_splits: u64,
        ) -> (Bucket, Bucket) {
            assert_eq!(
                yield_nft.resource_address(),
                self.yield_nft_raddy,
                "Please send yield nft"
            );

            let nft_proof: NonFungibleProof = yield_nft.create_proof_of_all();
            let checked_nft: CheckedNonFungibleProof = nft_proof.check(self.yield_nft_raddy);
            ...
            Runtime::emit_event(BurnYieldNFTEvent {
                ...
                burnt_hour_of_mint: data.hour_of_mint,
                burnt_n_super_minted: data.n_super_minted,
                burnt_n_trust_minted: data.n_trust_minted,
            });

            Runtime::emit_event(SplitNFTEvent {
                burnt_nft_id: time_of_mint,
                new_nft_ids: created_nft_ids,
            });
        ...
        pub fn check_and_burn_nft(
            ...
            Runtime::emit_event(BurnYieldNFTEvent {
                burnt_nft_id: local_id,
                burnt_hour_of_mint: nft_data.hour_of_mint,
                burnt_n_super_minted: nft_data.n_super_minted,
                burnt_n_trust_minted: nft_data.n_trust_minted,
            });
        ...
        pub fn vested_withdraw(&mut self) -> Bucket {
            assert!(!self.sale_completed, "Token Sale is not yet complete!");
        ...
        pub fn calculate_yield_curve_for_hour(&self, hour: u64) -> Decimal {
            ...
            let term_2_numerator: Decimal = EULER.checked_mul(757811).unwrap();
