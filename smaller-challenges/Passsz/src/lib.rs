...
blueprint! {
    ...
    impl Escrow {
        ...
        pub fn instantiate_escrow(
            ...
            assert!(to_be_paid_by_party_1.validate().is_ok(), "First resoure spec not valid");
            
            assert!(to_be_paid_by_party_2.validate().is_ok(), "Second resoure spec not valid");
        
            assert_ne!(
                to_be_paid_by_party_1, to_be_paid_by_party_2,
                "The two resource spcefiers can not equal one another"
            );
            ...
            let escrow_obligatons: Bucket = ResourceBuilder::new_non_fungible()
                .metadata("name", "Escrow Obligation")
                .metadata("symbol", "ESCROW")
                .metadata("team-member-1-github-username", "PKarics")
                .metadata("team-member-2-github-username", "zterhes")
                .metadata("team-member-3-github-username", "kovarik85")
                .metadata("team-member-4-github-username", "laszlocph")
            ...
            let component_address: ComponentAddress = Self {
                vaults,
                obligation_non_fungible_resource: escrow_obligatons.resource_address(),
                is_escrow_fulfilled: false
            }
            .instantiate()
            .globalize();

            (component_address, escrow_obligatons)
        }
        ...
        pub fn withdraw(&mut self, obligation_badge: Proof) -> Bucket {
            assert!(
                ...
                "You can not withwraw ypur funds unless the escrow noot fulfilled!"
            );
