...
blueprint! {
    ...
    impl Escrow {
        ...
        pub fn instantiate_escrow(
            ...
            assert!(to_be_paid_by_party_1.validate().is_ok(),"The first resource bad");
            assert!(to_be_paid_by_party_2.validate().is_ok(),"Second resource bad");
            assert_ne!(to_be_paid_by_party_1.resource_address(),to_be_paid_by_party_2.resource_address(),"the two resources are the same");
            let escrow_obligation1 = EscrowObligation {amount_to_pay: to_be_paid_by_party_1.clone(), amount_to_get: to_be_paid_by_party_2.clone()};
            let escrow_obligation2 = EscrowObligation {amount_to_pay: to_be_paid_by_party_2.clone(), amount_to_get: to_be_paid_by_party_1.clone()};

            let escrow_bucket : Bucket = ResourceBuilder::new_non_fungible()
            .metadata("name", "Escrow obligation")
            .metadata("Symbol", "ESCROW")
            .metadata("description","I am an escrow obligation")
            .metadata("team-member-1-github-username", "otottkovi2")
            .metadata("team-member-2-github-username", "zoklu")
            .metadata("team-member-3-github-username", "kohazidaniel")
            .metadata("team-member-4-github-username", "phecker6")
            .initial_supply([
                (
            ...
            vaults.insert(to_be_paid_by_party_1.clone(), Vault::new(to_be_paid_by_party_1.resource_address()));
            vaults.insert(to_be_paid_by_party_2.clone(), Vault::new(to_be_paid_by_party_2.resource_address()));
            let component_address : ComponentAddress = Self{ vaults,obligation_non_fungible_resource:escrow_bucket.resource_address(),is_escrow_fulfilled: false}
            .instantiate()
            .globalize();

            (component_address,escrow_bucket)
        }
        ...
        pub fn deposit(&mut self, obligation_badge: Proof, mut funds: Bucket) -> Bucket {
            let obligation_badge = obligation_badge
            .validate_proof(self.obligation_non_fungible_resource)
            .expect("u can't do that w/ ur rights");

            let obligation: EscrowObligation = obligation_badge.non_fungible().data();
            let vault = self.vaults.get_mut(&obligation.amount_to_pay).unwrap();
            let funds_to_deposit = match obligation.amount_to_pay{
        ...
        pub fn withdraw(&mut self, obligation_badge: Proof) -> Bucket {
            assert!(self.is_escrow_fulfilled(),"We're not done yet");
            let obligation_badge = obligation_badge.validate_proof(self.obligation_non_fungible_resource).expect("u can't do that w/ ur rights");
            let obligation : EscrowObligation = obligation_badge.non_fungible().data();
            let vault = self.vaults.get_mut(&obligation.amount_to_get).unwrap();
            vault.take_all()
        }
