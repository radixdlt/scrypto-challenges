...
blueprint! {
    ...
    impl Escrow {
        ...
        pub fn instantiate_escrow(
            ...
            assert!(
                to_be_paid_by_party_2.validate().is_ok(,
                "second resource is invalid")
            )
            assert_ne!(
                to_be_paid_by_party_1, to_be_paid_by_party_2, "The 2 resources cannot be equal"
            )
            ...
            let escrow_obligations:Bucket=ResourceBuilder::new_non_fungible()
                ...
                .metadata("team-member-2-github-username ", "pjfvelho")
                .metadata("team-member-3-github-username ", "IDC87")
                .metadata("team-member-4-github-username ", "CristianoMMM")
                .initial_supply([
                    (
                        NonFungibleId::from_u32(1), // 1u32
                        party_1_obligation
                    ),
                    (
                        NonFungibleId::from_u32(2), // 2u32
                        party_2_obligation
                    ),
        ...
        pub fn deposit(&mut self, obligation_badge: Proof, mut funds: Bucket) -> Bucket {
            let obligation_badge: ValidatingProof = obligation_badge
                .validate_proof(self.obligation_non_fungible_resource)
                .expect("error\n badge provided not valid!");
            ...
            let funds_to_deposit: Bucket = match obligation.amount_to_pay {
                ...
                ResourceSpecifier:: NonFungible { non_fungible_ids, ..} => funds.take_non_fungible(&non_fungible_ids)
            };
        ...
        pub fn withdraw(&mut self, obligation_badge: Proof) -> Bucket {
            assert!(
                ...
                "Cannot withdraw funds without escrow has been fulfilled"
            );

            let obligation_badge: ValidateProof = obligation_badge
            .validate_proof(self.obligation_non_fungible_resource)
            .expect("Invalid badge");

            let obligation EscrowObligation = obligation_badge.non_fungible().1();
