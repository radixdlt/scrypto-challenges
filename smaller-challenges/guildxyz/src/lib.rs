   ...
   impl TokenSale {
        pub fn new(price_per_token: Decimal) -> (ComponentAddress, Bucket) {
            let bucket: Bucket = ResourceBuilder::new_fungible()
                .metadata("name", "Guildxyz Token")
                .metadata("team-member-1-ticket-number", "#4075482339")
                .metadata("team-member-2-ticket-number", "#4033522279")
            ...
            let component_address: ComponentAddress = Self {
                tokens_vault: Vault::with_bucket(bucket),
        ...
        pub fn buy(&mut self, funds: Bucket) -> Bucket {
            ...
            self.tokens_vault.take(purchase_amount)
        }
