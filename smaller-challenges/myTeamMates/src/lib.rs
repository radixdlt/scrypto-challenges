   ...
   impl Escrow {
       ...
       pub fn instantiate_escrow(
        ...
        assert!(
            ...
            "First resource is not valid"
        );
        assert!(
            to_be_paid_by_party_2.validate().is_ok(),
            "Second resource is not valid"
        );

        assert_ne!(
            to_be_paid_by_party_1, to_be_paid_by_party_2,
            "The two resouurce specifier can not equal"
        ...
        let escrow_obligations: Bucket = ResourceBuilder::new_non_fungible()
            ...
            .metadata("description", "This resource description")
            .metadata("team-member-1-github-username", "shghs123")
            .metadata("team-member-2-github-username", "folsav")
            .metadata("team-member-3-github-username", "jtokes")
            .metadata("team-member-4-github-username", "vologya")
            .initial_supply([
                (
            ...
            let component_addres: ComponentAddress = Self {
                vaults,
            ...
            (component_addres, escrow_obligations)
       }
       ...
       pub fn withdraw(&mut self, obligation_badge: Proof) -> Bucket {
        assert!(
            self.is_escrow_fulfilled(),
            "You can not withdraw your funds unless the escrow has benne concluded."
        );
