    ...
    impl TokenSale {
        pub fn new(price_per_token: Decimal) -> (ComponentAddress, Bucket) {
            let bucket: Bucket = ResourceBuilder::new_fungible()
                .metadata("name", "Vienna City Token")
                .metadata("symbol", "VCT")
                .metadata("team-member-1-ticket-number", "4104866319")
            ...
            let component_address: ComponentAddress = Self {
                vienna_city_token_vault: Vault::with_bucket(bucket),
        ...
        pub fn buy(&mut self, funds: Bucket) -> Bucket {
            ...
            self.vienna_city_token_vault.take(purchase_amount)
        }
