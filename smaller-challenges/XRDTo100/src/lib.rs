    ...
    impl TokenSale {
        pub fn new(price_per_token: Decimal) -> (ComponentAddress, Bucket) {
            let bucket: Bucket = ResourceBuilder::new_fungible()
                .metadata("name", "XRDTo100")
                .metadata("symbol", "USEFUL")
                .metadata("team-member-1-ticket-number", "4024468549")
                .metadata("team-member-2-ticket-number", "4137830159")
                .metadata("team-member-3-ticket-number", "4101618659")
                .metadata("team-member-4-ticket-number", "4123184649")
