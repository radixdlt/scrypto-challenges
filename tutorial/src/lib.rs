...
mod parametric_insurance {
    ...
    impl ParametricInsurance {
        ...
        pub fn new(insured_domain: String, premium_amount: u64, payout_amount: u64, risk:u8) -> (Global<ParametricInsurance>, Bucket) {
            let in_badge = ResourceBuilder::new_integer_non_fungible::<InBadge>(OwnerRole::None) 
            .metadata(metadata! {
                 init {
                   "name" => "InfiniX", locked;
                   "description" => "Here is your proof of insurance!", locked;
                 }
            })
            .mint_initial_supply([
                (1.into(), InBadge { 
                    insured_domain: insured_domain.clone(),
                    risk_level: risk   
                })
            ]);
        ...
        pub fn is_claimed(&self, proof:Proof) -> bool {
            proof.check(self.nft.address());
            self.is_claimed
